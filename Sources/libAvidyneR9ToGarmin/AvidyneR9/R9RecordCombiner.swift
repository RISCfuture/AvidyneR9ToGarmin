...
class R9RecordCombiner<RecordType: R9Record> {
    ...
    func get(_ keyPath: KeyPath<RecordType, Array<Float?>>) -> Array<Float?> {
    ...
    func average<T>(_ values: Array<T>, strategy: AverageStrategy) -> T? where T: BinaryInteger {
        switch strategy {
            case .mean:
    ...
    func average(_ values: Array<Float>, strategy: AverageStrategy) -> Float? {
        switch strategy {
            case .mean:
                return values.reduce(0, +) / Float(values.count)
            case .mode:
...
fileprivate let north = Vector2(1, 0)

fileprivate func deg2rad(_ deg: Float) -> Float {
    deg * .pi / 180
}

fileprivate func rad2deg(_ rad: Float) -> Float {
    rad * 180 / .pi
}

fileprivate func /<T: BinaryInteger>(lhs: Vector2, rhs: T) -> Vector2 {
    return .init(lhs.x / Float(rhs), lhs.y / Float(rhs))
}

fileprivate func /(lhs: Vector2, rhs: Float) -> Vector2 {
